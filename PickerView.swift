...
struct PickerView: View {
    ...
    @State private var displayTeams: [[String]] = []
        ...
        VStack(spacing: 8) {
            ...
            if index < displayTeams.count {
                VStack(alignment: .leading, spacing: 4) {
                    ForEach(displayTeams[index], id: \.self) { name in
                        Text(name)
                            .font(.body)
    ...
    private func scheduleNextTick() {
        // easeOut: interval grows as we approach totalTicks
        let progress = Double(tickCount) / Double(totalTicks)
...
#Preview {
    PickerView(
        model: {
            ...
            m.participants = ["Alice", "Bob", "Carol", "Dave", "Eve", "Frank"]
            m.numberOfTeams = 3
            return m
        }(),
